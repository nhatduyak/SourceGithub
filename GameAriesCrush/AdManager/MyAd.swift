...
@objc class MyAd:NSObject,AmazonAdInterstitialDelegate,AmazonAdViewDelegate,AppModuleDelegate {
    ...
    let adColonyAppID = "appbdee68ae27024084bb334a"
    let adColonyZoneID = "vzf8fb4670a60e4a139d01b5"
    ...
    func ViewDidload()
    {
        do{
            ...
            if(Utility.isAd4 || Utility.isAd7 || Utility.isAd5 || Utility.isAd8 || Utility.isAd2)
            {
        ...
        }catch{NSLog("Unexpected error: \(error).")
        }
    ...
    func loadAmazonAdWithUserInterfaceIdiom(userInterfaceIdiom: UIUserInterfaceIdiom, interfaceOrientation: UIInterfaceOrientation) -> Void {
        ...
        }catch{print("loi load amazon")}
    }
    func timerMethodAutoAmazon(timer:NSTimer) {
        print("auto load amazon")
        do{
            loadAmazonAdWithUserInterfaceIdiom(UIDevice.currentDevice().userInterfaceIdiom, interfaceOrientation: UIApplication.sharedApplication().statusBarOrientation)
            let Version = Utility.abc.platformNiceString()
            if(Version == "CDMA")
            {
                loadAmazonFull()
        ...
        }catch{print("loi load amzon ads")}
    ...
    func loadAmazonFull()
    {
        ...
        }catch{print("Loi load amazon full")}
    }
    func showAmazonFull()
    {
        do{
            interstitialAmazon.presentFromViewController(self.viewController)
        }catch{print("loi show full amazon")}
