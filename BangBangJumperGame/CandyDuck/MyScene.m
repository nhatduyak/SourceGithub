...
#import "BangBangJumperGame-Swift.h"
...
-(void)addReadyGameView{
    ...
    if(readyGameView==nil){
        ...
        [gameOverView.playButton addTarget:self action:@selector(playButtonPresseddragout:) forControlEvents:UIControlEventTouchDragOutside];
...
-(void)playButtonPresseddragout:(id)sender{
    [self showConfig];
}
...
- (void) adView:()banner didFailToReceiveAdWithError:()error{
    [self hideTopBanner:banner];
}

- (void) adViewDidReceiveAd:()banner{
